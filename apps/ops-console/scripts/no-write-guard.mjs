#!/usr/bin/env node
// No-write-guard (spec/15 read-only-default rule, bishomiq pattern).
//
...
for (const file of walk(SRC)) {
  ...
  lines.forEach((line, i) => {
    ...
    if (/\bfetch\s*\(/.test(line) && file !== CLIENT && !file.endsWith("client.ts")) {
      // fetch() itself outside client.ts is allowed only if it cannot carry a
      // mutation verb on the same statement; verb check above is the gate.
    }
  });
}
